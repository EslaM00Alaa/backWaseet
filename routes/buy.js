...
const  
     ...
     router.post ("/buy",sanitizeInput,async(req,res)=> {
        try {
            const { error } = validateSellOrder(req.body);
            if (error) return res.status(404).json({ msg: error.details[0].message });

            await client.query("INSERT INTO buyorders (name, phone,house_details,address_details,expected_price) VALUES ($1, $2 ,$3 ,$4 ,$5)", [req.body.name, req.body.phone,req.body.house_details,req.body.address_details,req.body.expected_price]);
            res.json({ msg: "ok" });
     ...
     router.post("/get", async (req, res) => {
        try {
          ...
          let sql = `SELECT * FROM buyorders ${conditionStr} ORDER BY marked ASC;`;
      ...
      router.put("/mark/:id", async (req, res) => {
        try {
          ...
          await client.query("UPDATE buyorders SET marked = $1 WHERE id = $2;", [req.body.marked, req.params.id]);
          res.json({ msg: "ok" });
        } catch (error) {
          return res.status(404).json({ msg: error.message });
        }
